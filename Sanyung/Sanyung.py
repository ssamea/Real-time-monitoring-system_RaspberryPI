...
path_on_cloud ="food_img/Sanyung/"
...
#firebase 데이터 값 가져오기
db_menu = db.reference("Menu_DB/Sanyung").get()

for R_key in db_menu:
    price.append(db.reference("Menu_DB/Sanyung/%s/price" %R_key).get())
    name.append(db.reference("Menu_DB/Sanyung/%s/menu_name" % R_key).get())
...
class MyWindow(QWidget): #GUI창 설정
    ...
    def btn_order(self):
        ...
        if (self.tableWidget.item(0, 0) is None): #첫번째 줄이 비어있으면
        ...
        elif (self.tableWidget.item(1, 0) is not None and self.tableWidget.item(0, 0) is not None # 두번째줄까지 비어있지 않으면
            ...
            db.reference('Restaurant_DB/Real_time/Sanyung').push({
              ...
              self.tableWidget.item(0, 0) is not None and self.tableWidget.item(3, 0) is None): #세번째 줄까지 비어있지 않으면
