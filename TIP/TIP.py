...
#firebase 데이터 값 가져오기
db_menu = db.reference("Menu_DB/TIP").get()

for R_key in db_menu:
    price.append(db.reference("Menu_DB/TIP/%s/price" %R_key).get())
    name.append(db.reference("Menu_DB/TIP/%s/menu_name" % R_key).get())
